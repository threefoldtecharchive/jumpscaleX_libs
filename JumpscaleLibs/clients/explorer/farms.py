...
class Farms:
    ...
    def register(self, farm):
        resp = self._session.post(self._client.url + "/farms", json=farm._ddict)
        return resp.json()["id"]
    ...
    def update(self, farm, identity=None):
        ...
        self._session.put(self._client.url + f"/farms/{farm.id}", auth=auth, headers=headers, json=farm._ddict)
        return True
    ...
    def delete(self, farm_id, node_id, identity=None):
        ...
        self._session.delete(self._client.url + f"/farms/{farm_id}/{node_id}", auth=auth, headers=headers)
        return True
    ...
    def get(self, farm_id=None, farm_name=None):
        ...
        resp = self._session.get(self._client.url + f"/farms/{farm_id}")
