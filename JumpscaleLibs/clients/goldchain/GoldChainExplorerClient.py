...
class GoldChainExplorerClient(j.baseclasses.object):
    """
    Client to get data from a goldchain explorer.
    """
    ...
    def get(self, addresses, endpoint):
        ...
        for idx in indices:
            try:
                ...
                if resp.getcode() == 204:
                    raise j.clients.goldchain.errors.ExplorerNoContent(
                        "GET: no content available (code: 204)", endpoint
                    )
                raise j.clients.goldchain.errors.ExplorerServerError(
                    "error (code: {})".format(resp.getcode()), endpoint
                )
                ...
                self._log_debug(
                    "goldchain explorer get exception at endpoint {} on {}: {}".format(endpoint, address, e)
                )
                pass
        raise j.clients.goldchain.errors.ExplorerNotAvailable(
    ...
    def post(self, addresses, endpoint, data):
        ...
        for idx in indices:
            try:
                ...
                if resp.getcode() == 200:
                    return resp.readline()
                raise j.clients.goldchain.errors.ExplorerServerPostError(
