...
def main(self):
    ...
    kosmos 'j.clients.goldchain.test(name="transaction_get")'
    """
    ...
    assert (
        txn.version == 1
    )  # in reality it is 0, but the JSX goldchain client converts v0 transactions automatically to v1 transactions
    ...
    # the explorer provides us also with info that is not part of a goldchain txn,
    # the goldchain client injects this into the txn as well for easy look-up
