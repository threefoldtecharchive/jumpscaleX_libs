...
class BinaryData(BaseDataTypeClass):
    """
    BinaryData is the data type used for any binary data used in goldchain.
    """
...
class Hash(BinaryData):
    ...
    """
    GoldChain Hash Object, a special type of BinaryData
    """
...
class Currency(BaseDataTypeClass):
    ...
    def value(self, value):
        ...
        if isinstance(value, (int, str, Decimal)):
            if isinstance(value, str):
                ...
                if len(value) >= 4 and value[-3:] == "GFT":
                    value = value[:-3].rstrip()
            d = Decimal(value)
            sign, _, exp = d.as_tuple()
            if exp < -9:
                raise j.clients.goldchain.errors.CurrencyPrecisionOverflow(d)
            if sign != 0:
                raise j.clients.goldchain.errors.CurrencyNegativeValue(d)
            self._value = d
            return
    ...
    def str(self, with_unit=False):
        """
        Turn this Currency value into a str GFT unit-based value,
        optionally with the currency notation.

        @param with_unit: include the GFT currency suffix unit with the str
        """
