...
class TransactionV128(TransactionBaseClass):
    ...
    def _from_json_data_object(self, data):
        ...
        self._mint_condition = j.clients.goldchain.types.conditions.from_json(data.get("mintcondition", {}))
        self._mint_fulfillment = j.clients.goldchain.types.fulfillments.from_json(data.get("mintfulfillment", {}))
...
class TransactionV129(TransactionBaseClass):
    ...
    def _binary_encode_data(self):
        encoder = j.data.rivine.encoder_sia_get()
        encoder.add_array(self._nonce.value)
        encoder.add_all(self.mint_fulfillment, self._coin_outputs, self.miner_fees, self.data)
        return encoder.data
...
class TransactionV130(TransactionBaseClass):
    _SPECIFIER = b"coin destruct tx\0"
    ...
    def _id_input_compute(self):
        return bytearray(TransactionV130._SPECIFIER) + self._binary_encode_data()

    def _binary_encode_data(self):
        encoder = j.data.rivine.encoder_rivine_get()
        encoder.add_all(self.coin_inputs, self._coin_outputs)
