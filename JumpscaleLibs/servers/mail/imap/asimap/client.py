...
# asimapd imports
#
from . import mbox as asmbox, throttle
from .exceptions import No, Bad, MailboxInconsistency, MailboxLock
...
class PreAuthenticated(BaseClientHandler):
    ...
    ##################################################################
    #
    def __init__(self, client, auth_system, models):
        """
        Arguments:
        - `client`: An asynchat.async_chat object that is connected to the IMAP
                    client we are handling. This lets us send messages to that
    ...
    def do_login(self, cmd):
        ...
        try:
            ...
            #
            throttle.login_failed(cmd.user_name, self.client.rem_addr)
            raise No(str(e))
        return None
...
class Authenticated(BaseClientHandler):
    ...
    ##################################################################
    #
    def __init__(self, client, user_server, models):
        """
    ...
    def do_create(self, cmd):
        ...
        if self.process_or_queue(cmd, queue=False):
            try:
                ...
                pass
        asmbox.Mailbox.create(cmd.mailbox_name, self.server)
        return
    ...
    def do_delete(self, cmd):
        ...
        if self.process_or_queue(cmd, queue=False):
            try:
                ...
                pass
        asmbox.Mailbox.delete(cmd.mailbox_name, self.server)
        return
