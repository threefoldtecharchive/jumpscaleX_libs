...
#
flag_to_str = {REPLACE_FLAGS: "FLAGS", ADD_FLAGS: "+FLAGS", REMOVE_FLAGS: "-FLAGS"}
...
system_flags = ["\\answered", "\\flagged", "\\deleted", "\\seen", "\\draft", "\\recent"]
...
_date_month = r"(Jan)|(Feb)|(Mar)|(Apr)|(May)|(Jun)|(Jul)|(Aug)|(Sep)|" + r"(Oct)|(Nov)|(Dec)"
_time = r"\d\d:\d\d:\d\d"
_zone = r"[-+]\d\d\d\d"
_date_time = (
...
class IMAPClientCommand(object):
    ...
    def __str__(self):
        ...
        if self.command is not None:
            ...
            if self.command == "fetch":
                ...
                result.append("(%s)" % ", ".join("%s:'%s'" % (x, y) for x, y in self.id_dict.items()))
    ...
    def _parse(self):
        ...
        #
        self.tag = self._p_re(_tag_re, syntax_error="missing expected " "tag that prefixes a command")
        self._p_simple_string(" ", syntax_error="expected ' ' after tag")
        self.command = self._p_re(_atom_re, syntax_error="expected an atom for " "the command name").lower()
    ...
    def _p_id(self):
        ...
        if self._p_simple_string("nil", silent=True):
            return
        self._p_simple_string("(", swallow=False, syntax_error="expected " "a parenthesized list of key/value pairs")
    ...
    def _p_append(self):
        ...
        if self._p_simple_string("(", silent=True, swallow=False):
            ...
            self._p_simple_string(" ", syntax_error="expected ' ' after flag " "list")
        ...
        if self._p_simple_string('"', silent=True, swallow=False):
            ...
            self._p_simple_string(" ", syntax_error="expected ' ' after " "rfc822 date-time")
    ...
    def _p_search(self):
        ...
        #
        self.search_key = IMAPSearch("and", search_key=self._p_list_of(self._p_search_key))
    ...
    def _p_uid(self):
        ...
        if command not in uid_commands:
            raise BadSyntax("%s is not a valid UID command: %s" % (command, str(uid_commands)))
    ...
    def _p_paren_list_of(self, func):
        ...
        result = []
        self._p_simple_string("(", syntax_error="expected a '(' beginning " "a parenthesized list")
    ...
    def _p_fetch_atts(self):
        ...
        if self._p_simple_string("(", silent=True, swallow=False):
            ...
            if macro is not None:
                ...
                macro = macro.lower()
                if macro == "all":
                    return [FetchAtt("flags"), FetchAtt("internaldate"), FetchAtt("rfc822.size"), FetchAtt("envelope")]
                elif macro == "full":
                    return [
                        FetchAtt("flags"),
                        ...
                        FetchAtt("envelope"),
                        FetchAtt("bodystructure", ext_data=False, actual_command="BODY"),
                    ]
                elif macro == "fast":
                    return [FetchAtt("flags"), FetchAtt("internaldate"), FetchAtt("rfc822.size")]
                else:
                    raise BadSyntax(value='"%s" is not a valid fetch ' "attribute")
            else:
    ...
    def _p_fetch_att(self):
        ...
        if fetch_att_tok not in fetch_atts:
            raise BadSyntax("'%s' is not a valid FETCH argument" % fetch_att_tok)
        ...
        if fetch_att_tok not in ("body", "body.peek"):
            ...
            if fetch_att_tok == "rfc822":
                ...
                return FetchAtt("body", section=["header"], peek=True, actual_command="RFC822.HEADER")
            elif fetch_att_tok == "rfc822.text":
                return FetchAtt("body", section=["text"], actual_command="RFC822.TEXT")
            else:
                return FetchAtt(fetch_att_tok)

        if fetch_att_tok == "body" and self._p_simple_string("[", silent=True, swallow=False) is None:
            return FetchAtt("bodystructure", ext_data=False, actual_command="BODY")
        ...
        return FetchAtt(fetch_att_tok, section=section, partial=self._p_partial(), peek=peek)
    ...
    def _p_section(self):
        ...
        #
        section = None
        section_texts = ["header.fields.not", "header.fields", "header", "text"]
        if len(sect_list) > 0:
        ...
        if section is None:
            raise BadSyntax(
                value="%s: expected a valid section " "identifier, one of: %s" % (self.input[:10], str(section_texts))
            )
    ...
    def _p_search_key(self):
        ...
        if search_tok:
            ...
            if not hasattr(self, "_p_srchkey_%s" % search_tok):
                raise UnknownSearchKey(value='Unknown search key "%s"' % search_tok)
    ...
    def is_seq_num(self, val):
        ...
        if val.isdigit():
            ...
            if num < 0:  # 0 is a valid uid...
                raise SyntaxError("message sequence numbers " "must be greater then 0: %d" % num)
            return num
    ...
    def _p_srchkey_bcc(self):
        self._p_simple_string(" ")
        return IMAPSearch("header", header="bcc", string=self._p_astring().lower())
    ...
    def _p_srchkey_cc(self):
        self._p_simple_string(" ")
        return IMAPSearch("header", header="cc", string=self._p_astring().lower())
    ...
    def _p_srchkey_from(self):
        self._p_simple_string(" ")
        return IMAPSearch("header", header="from", string=self._p_astring().lower())
    ...
    def _p_srchkey_header(self):
        ...
        self._p_simple_string(" ")
        return IMAPSearch("header", header=header_fld_name, string=self._p_astring().lower())
    ...
    def _p_srchkey_new(self):
        return IMAPSearch("and", search_key=[self._p_srchkey_recent(), self._p_srchkey_unseen()])
    ...
    def _p_srchkey_subject(self):
        self._p_simple_string(" ")
        return IMAPSearch("header", header="subject", string=self._p_astring().lower())
    ...
    def _p_srchkey_to(self):
        self._p_simple_string(" ")
        return IMAPSearch("header", header="to", string=self._p_astring().lower())
    ...
    def _p_msg_set(self):
        ...
        * is the largest number in use.  For message sequence numbers, it is
        ...
        #
        msg_set = self._p_re(_msg_set_re, syntax_error="missing or " "invalid message sequence set")
        ...
        for seq_num in seqs:
            ...
            #
            raise BadSyntax(value='"%s" is not a valid message ' "sequence number" % seq_num)
        return result
    ...
    def _p_date_time(self):
        ...
        date_time = self._p_re(_date_time_re, syntax_error="expected a " "rfc822 formated date-time")

        # We need to strip off the "" surrounding the date-time string.
        #
        return utils.parsedate(date_time[1:-1])
    ...
    def _p_status_att(self):
        ...
        for status in stats_atts:
            ...
            if stat is not None:
                return stat
        raise BadSyntax(value="expected a status attribute: %s" % str(stats_atts))
    ...
    def _p_string_nstring_pairs(self):
        ...
        """
        key = self._p_string()
        self._p_simple_string(" ", syntax_error="expected a space between " "strings in a parenthesized key/value list")
        ...
        try:
            ...
            if literal_length > len(self.input):
                raise BadLiteral(
                    ...
                    "long, expected at least %d" % (len(self.input), literal_length)
                )
    ...
    #######################################################################
    #
    def _p_re(self, regexp, silent=False, swallow=True, group=0, syntax_error=None):
        ...
        if match is None:
            if silent:
                ...
                if syntax_error:
                    ...
                    raise NoMatch(value="No match for r.e. '%s'" % regexp.pattern)
        if swallow:
    ...
    #######################################################################
    #
    def _p_simple_string(self, string, silent=False, swallow=True, case_matters=False, syntax_error=None):
        ...
        if match is None:
            if silent:
                ...
                if syntax_error:
                    ...
                    raise NoMatch(
                        value="No match for simple string '%s', input started with: '%s'" % (string, self.input[:10])
                    )
        if swallow:
