...
############################################################################
#
class BadSection(exceptions.Bad):
...
class HeaderGenerator(Generator):
    ...
    def _write_headers(self, msg):
        for h, v in list(msg.items()):
            ...
            #
            if (self._skip and h.lower() in self._headers) or (not self._skip and h.lower() not in self._headers):
                continue

            # Otherwise output this header.
            #
            print("%s:" % h, end=" ", file=self._fp)
            if self.maxheaderlen == 0:
                ...
                print(
                    Header(v, maxlinelen=self.maxheaderlen, header_name=h, continuation_ws="\t").encode(), file=self._fp
                )
...
class FetchAtt(object):
    ...
    #######################################################################
    #
    def __init__(self, attribute, section=None, partial=None, peek=False, ext_data=True, actual_command=None):
        """
    ...
    #
    def __repr__(self):
        return "FetchAtt(%s[%s]<%s>)" % (self.attribute, str(self.section), str(self.partial))
            ...
            if self.section is not None:
                ...
                for s in self.section:
                    ...
                    if isinstance(s, (list, tuple)):
                        sects.append("%s (%s)" % (str(s[0]).upper(), " ".join(x for x in s[1])))
                    else:
    ...
    def fetch(self, ctx):
        ...
        if self.attribute == "body":
            ...
            result = "(%s)" % " ".join([constants.seq_to_flag(x) for x in self.ctx.sequences])
    ...
    def body(self, msg, section):
        ...
        if len(section) == 0:
            ...
            if len(section) == 1:
                ...
                if isinstance(section[0], int):
                    ...
                    if section[0] == 1 and not msg.is_multipart():
                        ...
                        if section[0] != 1 and not msg.is_multipart():
                            raise BadSection(
                                "Trying to retrieve section %d " "and this message is not " "multipart" % section[0]
                            )
                        try:
                            msg = msg.get_payload(section[0] - 1)
                        except IndexError:
                            raise BadSection("Section %d does not exist in " "this message sub-part" % section[0])
                    ...
                    if section[0][0].upper() == "HEADER.FIELDS":
                        g = HeaderGenerator(fp, headers=section[0][1], skip=False)
                    elif section[0][0].upper() == "HEADER.FIELDS.NOT":
                        g = HeaderGenerator(fp, headers=section[0][1], skip=True)
                    else:
                        raise BadSection(
                    ...
                    if isinstance(section[0], str) and section[0].upper() == "MIME":
                        ...
                        self.log.warn("body: Unexpected section[0] value: %s" % repr(section))
                        raise BadSection("%s: Unexpected section value: %s" % (str(self), repr(section[0])))
                ...
                if not msg.is_multipart():
                    raise BadSection(
                        "Message does not contain subsection %d " "(section list: %s" % section[0], section
                    )
                try:
                    msg = msg.get_payload(section[0] - 1)
                except TypeError:
            ...
            #
            if g is None:
                raise BadSection("Section selector '%s' can not be parsed" % section)
            g.flatten(msg)
    ...
    def envelope(self, msg):
        ...
        from_field = ""
        for field in ("date", "subject", "from", "sender", "reply-to", "to", "cc", "bcc", "in-reply-to", "message-id"):
    ...
    def bodystructure(self, msg):
        ...
        if msg.is_multipart():
            ...
            if not self.ext_data:
                return '(%s "%s")' % ("".join(sub_parts), msg.get_content_subtype())
        ...
        result.append(str(len(fix_eols(payload))))
