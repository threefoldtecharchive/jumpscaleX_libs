...
class K8sGenerator:
    ...
    def add_master(self, reservation, node_id, network_name, cluster_secret, ip_address, size, sshkeys=[]):
        if size not in [1, 2]:
        ...
        master.size = size
        if not isinstance(sshkeys, list):
            sshkeys = [sshkeys]
        master.sshkeys = sshkeys

        return master

    def add_worker(self, reservation, node_id, network_name, cluster_secret, ip_address, size, master_ip, sshkeys=[]):
